...
def compare_records(
    ...
    if left.get("market") in ("spread", "total") or right.get("market") in (
        "spread",
        "total",
    ):
    ...
    for label, record in (("A", left), ("B", right)):
        ...
        if source is None:
            reasons.append(
                f"{label} has no source timestamp. Retrieval time cannot replace it."
            )
        elif source > now_minute:
            reasons.append(f"{label} has a future source timestamp.")
        elif now_minute - source > max_age:
            reasons.append(
                f"{label} is {now_minute - source} minutes old, outside your {max_age}-minute age limit."
            )
        if fetched is None:
    ...
    if (
        a_time is not None
        and b_time is not None
        and abs(a_time - b_time) > max_time_gap
    ):
        reasons.append(
            f"Source times are {abs(a_time - b_time)} minutes apart, beyond your {max_time_gap}-minute gap limit."
        )
    return reasons
...
def record_table(left: dict, candidates: list[dict]) -> str:
    ...
    for key, label in (
        ...
        ("source_minute", "Source time"),
        ("retrieved_minute", "Retrieved at"),
    ):
        values = []
...
def _():
    scenario = mo.ui.dropdown(
        options={
            ...
            "Wrong event": "event",
            "Different market": "market",
            "Other team's outcome": "outcome",
            "Different point / line": "point",
            "Different period": "period",
            "Different settlement rules": "settlement",
            "Unknown settlement rules": "unknown_rules",
            "Missing named outcome": "missing",
            "Duplicate candidate records": "duplicate",
            "Missing source timestamp": "unknown_time",
            "Future source timestamp": "future_time",
        },
        value="Different point / line",
        label="Change record B",
        full_width=True,
    )
    source_age = mo.ui.slider(
        0, 20, value=1, label="B source age (minutes)", show_value=True
    )
    max_age = mo.ui.slider(
        0, 15, value=5, label="Your maximum age (minutes)", show_value=True
    )
    max_gap = mo.ui.slider(
        0, 15, value=2, label="Your maximum time gap (minutes)", show_value=True
    )
...
def _(max_age, max_gap, scenario, source_age):
    ...
    reasons = compare_records(
        left_record, right_candidates, max_age.value, max_gap.value
    )
    _verdict = (
        "Cannot establish comparability"
        if reasons
        else "Comparable under these teaching assumptions"
    )
    ...
    mo.vstack(
        [
            mo.Html(
                f"<div class='lab-result {'warn' if reasons else ''}' role='status'><strong>{_verdict}</strong><ul>{_explanation}</ul></div>"
            ),
            mo.Html(record_table(left_record, right_candidates)),
            mo.md(
                "All times belong to one fictional day. Comparison time is **12:00**. Both records were retrieved at 12:00; a recent retrieval does not make an older source quote fresh."
            ),
        ]
    )
...
def _():
    interval = mo.ui.dropdown(
        options={"1 minute": 1, "5 minutes": 5, "15 minutes": 15, "60 minutes": 60},
        value="15 minutes",
    ...
    weight = mo.ui.slider(
        0, 10, value=3, label="Hypothetical credits per request", show_value=True
    )
